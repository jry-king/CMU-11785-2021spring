...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        self.convolutional_layers = [Conv1D(channels[i], channels[i+1], kernel_sizes[i], strides[i], conv_weight_init_fn, bias_init_fn) for i in range(self.nlayers)]
        self.flatten = Flatten()
        conv_output_size = input_width
        for i in range(self.nlayers):
            conv_output_size = (conv_output_size - kernel_sizes[i]) // strides[i] + 1
        self.linear_layer = Linear(channels[-1]*conv_output_size, num_linear_neurons, linear_weight_init_fn, bias_init_fn)
    ...
    def forward(self, x):
        ...
        out = x
        for i in range(self.nlayers):
            out = self.convolutional_layers[i](out)
            out = self.activations[i](out)
        out = self.linear_layer(self.flatten(out))
        self.output = out
        return self.output
