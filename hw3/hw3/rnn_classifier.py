...
class RNNPhonemeClassifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        for i in range(seq_len):
            hidden_step = np.zeros((self.num_layers, batch_size, self.hidden_size), dtype=float)
            for j in range(self.num_layers):
                input = x[:, i] if j == 0 else hidden_step[j-1]
                hidden_previous = self.hiddens[i][j]
                hidden_step[j] = self.rnn[j](input, hidden_previous)
            self.hiddens.append(hidden_step.copy())
    ...
    def backward(self, delta):
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for i in range(seq_len-1, -1, -1):
            dh_prev = np.zeros((self.num_layers, batch_size, self.hidden_size), dtype=float)
            for j in range(self.num_layers-1, -1, -1):
                dx_out, dh_out = self.rnn[j].backward(dh[j], self.hiddens[i+1][j], self.hiddens[i+1][j-1], self.hiddens[i][j]) \
                    if j > 0 else self.rnn[j].backward(dh[j], self.hiddens[i+1][j], self.x[:, i], self.hiddens[i][j])
                dh_prev[j] = dh_out
                if j > 0:
                    dh[j-1] += dx_out
            dh = dh_prev
