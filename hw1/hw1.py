...
class MLP(object):
    ...
    def __init__(self, input_size, output_size, hiddens, activations, weight_init_fn,
        ...
        sizes = [self.input_size] + hiddens + [self.output_size]
        self.linear_layers = [Linear(sizes[i], sizes[i+1], weight_init_fn, bias_init_fn) for i in range(self.nlayers)]

        # If batch norm, add batch norm layers into the list 'self.bn_layers'
        if self.bn:
            self.bn_layers = [BatchNorm(sizes[i+1]) for i in range(np.minimum(self.nlayers, self.num_bn_layers))]
    ...
    def forward(self, x):
        ...
        for i in range(self.nlayers):
            ...
            if(self.bn and i < len(self.bn_layers)):
                out = self.bn_layers[i](out, not self.train_mode)       # pass eval value
    ...
    def backward(self, labels):
        ...
        derivative = self.criterion.derivative()
